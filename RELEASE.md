...
Add your PyPI token to `~/.zshrc` (one-time setup):

```bash
echo "export UV_PUBLISH_TOKEN=pypi-XXX" >> ~/.zshrc
source ~/.zshrc
```

Generate or renew a token at: https://pypi.org/manage/account/token/
...
> Requires `UV_PUBLISH_TOKEN` to be set (see Prerequisites).
...
```bash
# Recompile if needed (after a reboot)
# git clone https://github.com/modelcontextprotocol/registry.git /tmp/mcp-registry && cd /tmp/mcp-registry && make publisher

/tmp/mcp-registry/bin/mcp-publisher publish
```

On first use after a reboot, authenticate with GitHub:
